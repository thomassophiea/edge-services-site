  ...
  async getStation(macAddress: string): Promise<Station> {
    try {
      ...
      if (error instanceof Error) {
        ...
        throw new Error(`Failed to load client details: ${error.message}`);
      }
      throw error;
    }
  }
  ...
  async getSites(): Promise<Site[]> {
    try {
      return await this.makeRequestWithRetry(async () => {
        ...
        // Handle various API response formats like ConfigureSites does
        let sitesList: any[] = [];
        ...
        // Process sites to match the Site interface
        const processedSites = sitesList.map(site => ({
          ...site,
          name: site.siteName || site.name, // Ensure both name fields are available
      ...
      if (error instanceof Error && 
          (error.message.includes('SUPPRESSED_ANALYTICS_ERROR') || 
           error.message.includes('SUPPRESSED_NON_CRITICAL_ERROR'))) {
        // Return empty array for suppressed errors instead of throwing
        console.log('Returning empty sites array for suppressed endpoint');
        return [];
      }
      throw error;
    }
