...
export function ConfigureSites({ onShowDetail }: ConfigureSitesProps) {
  ...
  const loadSites = async (retryCount = 0): Promise<void> => {
    ...
    try {
      ...
      setCurrentLoadingStep('Fetching client data...');

      // Fetch stations (clients) data and correlate with sites
      let clientCountsBySite: Map<string, number> = new Map();
      try {
        console.log('Fetching stations for client counts...');
        const stations = await apiService.getStationsWithSiteCorrelation();
        
        // Count clients per site
        stations.forEach(station => {
          const siteId = station.siteId || station.site_id;
          if (siteId) {
            clientCountsBySite.set(siteId, (clientCountsBySite.get(siteId) || 0) + 1);
          }
        });
        
        console.log('Client counts by site:', Object.fromEntries(clientCountsBySite));
      } catch (stationsError) {
        console.warn('Failed to fetch client data, client counts will be 0:', stationsError);
        // Continue with site loading even if client data fails
      }
      ...
      const processedSites = sitesList.map(site => {
        ...
        return {
          ...
          allClients: clientCountsBySite.get(site.id) || 0, // Use correlated client count 
      ...
      if (processedSites.length === 0) {
        ...
        const totalClients = processedSites.reduce((sum, site) => sum + (site.allClients || 0), 0);
        console.log(`Loaded ${processedSites.length} sites with ${totalClients} total clients successfully`);
        toast.success(`Loaded ${processedSites.length} sites successfully`, {
          description: totalClients > 0 ? `${totalClients} connected clients` : undefined
        });
      }
